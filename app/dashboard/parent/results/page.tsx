...
export default function ParentChildResultsPage() {
  ...
  useEffect(() => {
    api.get('/parent/child/results').then(r => setResults(r.data.data || [])).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Child's Results & Marks</h2>
          ...
          {examTypes.map(t => (
            <button key={t} onClick={() => setFilter(t)}
              className={`px-4 py-1.5 rounded-full text-xs font-medium capitalize transition-all ${
                filter === t ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-700'
              }`}>
              {t.replace('_', ' ')}
            </button>
          ))}
      ...
      {/* Average Score Card */}
      {avgPct && (
        <div className="card p-6 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40 border-emerald-100 dark:border-emerald-900">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center">
              <ChartBarIcon className="w-8 h-8 text-white" />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Child's Overall Average Score</p>
              <p className="text-4xl font-display font-black text-emerald-600 dark:text-emerald-400">{avgPct}%</p>
            </div>
          </div>
        </div>
      )}
