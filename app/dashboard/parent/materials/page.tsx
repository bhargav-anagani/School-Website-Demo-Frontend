...
export default function ParentMaterialsPage() {
  ...
  const fetchMaterials = async () => {
    try {
      const res = await api.get('/parent/materials');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Download learning resources and notes uploaded for your child's class.</p>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {materials.map((m) => (
          <div key={m.id} className="card p-6 flex flex-col h-full hover:shadow-lg transition-all group border-l-4 border-emerald-500">
            <div className="flex items-start justify-between mb-4">
               <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-900/40 rounded-xl flex items-center justify-center text-emerald-600 group-hover:scale-110 transition-transform">
               ...
               <a 
                 ...
                 className="p-2 bg-emerald-600 text-white rounded-xl shadow-md hover:bg-emerald-700 transition-colors flex items-center gap-1 text-xs font-bold px-3"
               >
        ...
        {materials.length === 0 && (
          <div className="md:col-span-3 py-20 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">No materials available yet.</p>
          </div>
        )}
      </div>
    </div>
